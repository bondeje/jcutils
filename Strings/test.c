...
int test_setup(void) {
	...
	for (size_t i = 0; i < nstrings; i++) {
		...
		nerrors += CHECK(dynamic_strings[i].size == static_strings[i].size, 
			"size mismatch between strings for string %s. expected %lld, found %lld\n",
...
int test_String_is_empty(void) {
	...
	for (size_t i = 0; i < nstrings; i++) {
		nerrors += CHECK(String_is_empty(&static_strings[i]) == (static_strings[i].size == 0),
			"failed to identify an empty string. expected %s, found %s\n",
...
int test_String_split(void) {
	...
	nerrors += CHECK(3 == count,
		"failed to split %s by %s to the correct number. expected %d, found %lld\n",
		path_raw, "/", 3, (long long)count);
